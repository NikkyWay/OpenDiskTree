...
After a successful full scan, OpenDiskTree keeps a local metadata index and listens to the public macOS FSEvents journal. **Scan or Update Full Disk** automatically chooses a fast update when a trustworthy baseline exists; **Full rescan from scratch** remains available in the New scan menu. A fast update loads one compact directory catalog, re-enumerates only changed branches and applies that small overlay directly to the current snapshot. It does not copy millions of unchanged rows into a second database snapshot. Changed paths are sorted once and matched by binary search, while per-directory rollups supply unchanged subtree totals in constant time. Hard-linked files remain reusable too: one final reconciliation assigns their physical blocks to exactly one path, including links that cross changed and reused branches. The journal ignores OpenDiskTree's own database writes, persists an exact event boundary and keeps newer events queued for the following update. A quiet no-change update returns without writing another snapshot. If more than 25,000 paths changed, the primary action chooses a clean Turbo scan because a broad overlay can cost more than a fresh traversal; the explicit fast-update menu item remains available. Removed branches become lightweight tombstones during the visible update. The temporary overlay is hidden as soon as the updated base snapshot is published; its cascade deletion and tombstone reclamation run later during idle maintenance. Dropped, wrapped or unavailable event history still forces a conservative full scan. There is no private APFS parser or undocumented filesystem access, so the app remains compatible with FileVault, APFS volume groups and future macOS updates.

The app starts a separate quick overview while the exact scan is preparing. It lists the selected directory immediately using the same native bulk metadata request, shows sizes for entries that are files, labels folder totals as partial estimates, and never writes those provisional rows into the exact SQLite snapshot. The exact scan then enumerates every visible filesystem object and replaces the overview with real files, statuses and metadata. Scanner callbacks do not wait for SwiftUI layout, and an incremental pass does not rebuild the table or treemap until its database overlay is complete. Reusable directories are matched synchronously and written in large batches instead of paying an actor hop and SQLite flush per folder. Turbo is the default; Balanced remains available when lower I/O pressure matters more than elapsed time. macOS does not expose a public Windows-MFT equivalent, so timings still depend on the number of directories, permissions, cache state and SSD. On the development Mac, the reproducible production-scanner benchmark built a finalized 1.96-million-item snapshot in 24.15 seconds. Loading its 364,067-directory reuse catalog and completing an unchanged incremental overlay took 1.66 seconds. The latest installed-app full scan took 46 seconds, so the isolated number is a target and not a claim about every launch or volume.
